...
def updateDistance():
    ...
    while True:
        try:
            temp = sonar1.distance_metric(sonar1.raw_distance(2, 0.08))  #超声波测量距离 
            TEMP = sonar2.distance_metric(sonar2.raw_distance(2, 0.08))
            distance = temp if temp <= 450.0 else 0.0  #限幅， 超过450 的就等于0, 0代表就距离超出量程
            DISTANCE = TEMP if TEMP <= 450.0 else 0.0
        except Exception as e:
            print(e)
